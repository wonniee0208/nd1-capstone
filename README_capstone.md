...
3. `item` 값 검증
   ...
   * `tool_box`
   * `sensor_box`
4. `sim_mode=True`에서는 실제 로봇팔 제어 없이 성공 반환
5. 작업 성공 시 `/grasp_result`에 `true` 발행
6. 잘못된 `op` 또는 `item`이면 `/grasp_result`에 `false` 발행

이번 Node C 수정은 실제 로봇팔 제어 로직을 완성한 것이 아니라, Coordinator와 Node C 사이의 `/grasp_request` 토픽 계약을 맞추기 위한 통합 인터페이스 수정이다.
추후 실제 로봇팔 제어 또는 IK 로직을 구현하더라도 `/grasp_request`의 `op`, `item` 형식은 유지해야 한다.
...
본 프로젝트는 창고 피킹 보조 로봇을 가정한다.

사용자는 자연어로 물품 운반 명령을 내리고, 로봇은 지정된 선반에서 물품을 집어 작업자 또는 작업대로 전달한다.
...
이후 Coordinator는 다음 순서로 미션을 수행한다.
